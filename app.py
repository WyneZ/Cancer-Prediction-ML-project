...
#     st.write(f"Probability of Malignancy: {proba:.2%}")
#     st.write(f"Diagnosis: {'Malignant' if prediction == 1 else 'Benign'}")
    
#     # SHAP Explanation - FIXED VERSION
#     st.subheader("Feature Impact Analysis")
#     explainer = shap.TreeExplainer(model)
    
#     # Calculate SHAP values - different approach for single prediction
#     shap_values = explainer(input_df)
    
#     # Visualization options - choose one:
    
#     # OPTION 1: Force plot (best for single prediction)
...
#     # OPTION 2: Waterfall plot (alternative)
#     fig, ax = plt.subplots()
...
#     indices = np.argsort(importances)[-10:]  # Now works with np imported
